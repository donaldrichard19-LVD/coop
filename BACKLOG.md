...
- **RCS confirm-chip template not provisioned** — `RCS_TEMPLATE_CONFIRM_MERCHANT_SID` (the new template the confirm flow needs) isn't set on Render. Needs `npm run provision:rcs` run with real Twilio credentials (creates the `coop_rcs_confirm_merchant` Content API template, per `rcs/templates.js`), then the resulting SID added to Render's env vars. Until then, the confirm flow degrades gracefully — logs "would have sent" instead of texting the chip — same no-Twilio-creds no-op every other RCS send already falls back to, so nothing breaks, it's just not live yet.
